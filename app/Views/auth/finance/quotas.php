...
function loadTable() {
    $.post(apiBase + '/list', { type: currentFilter }, function(response) {
        ...
        if (response.status === 'success' && Array.isArray(response.data)) {
            response.data.forEach(function(row) {
                ...
                rows.push([
                    ...
                    parseFloat(row.amount_bs || 0).toLocaleString('es-VE', {minimumFractionDigits: 2}),
                    paymentTypeName(row.payment_type_id),
        ...
        else dt = $('#quotasTable').DataTable({ data: rows, pageLength: 25, language: {url: '//cdn.datatables.net/plug-ins/1.13.7/i18n/es-ES.json'}, dom: 'Bfrtip', buttons: ['excel', 'pdf'] });
    });
}
...
$('#quotaForm').submit(function(e) {
    ...
    $.ajax({
        ...
        success: function(res) {
            if (res.status === 'success') { $('#quotaModal').modal('hide'); loadTable(); }
