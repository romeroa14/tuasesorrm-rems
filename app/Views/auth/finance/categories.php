        ...
        <div class="card-body">
            <div class="table-responsive">
                <table id="dataTable" class="table table-striped table-bordered" style="width:100%">
                    <thead class="thead-light"><tr><th>ID</th><th>Nombre</th><th>Tipo</th><th>Descripción</th><th>Categoría Padre</th><th>Acciones</th></tr></thead>
                </table>
            </div>
        </div>
    </div>
...
<div class="modal fade" id="financeModal" tabindex="-1" role="dialog">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header bg-primary text-white"><h5 class="modal-title" id="modalTitle">Nueva Categoría</h5><button type="button" class="close text-white" data-dismiss="modal">&times;</button></div>
            <form id="financeForm">
                <div class="modal-body">
                    ...
                    <div class="row">
                        <div class="col-md-6"><div class="form-group"><label>Tipo <span class="text-danger">*</span></label><select class="form-control" id="type" name="type" required><option value="">Seleccionar...</option><option value="income">Ingreso</option><option value="expense">Gasto</option></select></div></div>
                        <div class="col-md-6"><div class="form-group"><label>Categoría Padre</label><select class="form-control" id="parent_id" name="parent_id"><option value="">Ninguna</option></select></div></div>
                    </div>
...
function loadTable() {
    $.post(apiBase, function(r) {
        ...
        if(r.status==='success'&&Array.isArray(r.data)) r.data.forEach(function(d){
            rows.push([d.id, d.name, '<span class="badge badge-'+ (d.type==='income'?'success':'danger')+'">'+(d.type==='income'?'Ingreso':'Gasto')+'</span>', d.description||'—', d.parent_id||'—',
                '<button class="btn btn-info btn-sm mr-1" onclick="edit('+d.id+')"><i class="fas fa-edit"></i></button><button class="btn btn-danger btn-sm" onclick="remove('+d.id+')"><i class="fas fa-trash"></i></button>']);
        });
        if(dt) dt.clear().rows.add(rows).draw(); else dt=$('#dataTable').DataTable({data:rows, pageLength:25, language:{url:'//cdn.datatables.net/plug-ins/1.13.7/i18n/es-ES.json'}});
    });
}
function showModal(mode,id){$('#record_id').val('');$('#financeForm')[0].reset();$('#modalTitle').text(mode==='create'?'Nueva Categoría':'Editar Categoría');$.post(apiBase,function(r){var sel=$('#parent_id').empty().append('<option value=\"\">Ninguna</option>');if(r.status==='success')r.data.forEach(function(d){sel.append('<option value=\"'+d.id+'\">'+d.name+'</option>');});if(mode==='edit'&&id)$.post(apiBase+'/'+id,function(r2){if(r2.status==='success'){var d=r2.data; Object.keys(d).forEach(function(k){var el=$('[name="'+k+'"]');if(el.length)el.val(d[k]);});$('#record_id').val(d.id);}});});$('#financeModal').modal('show');}
