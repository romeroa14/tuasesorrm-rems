<div class="container-fluid">
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
        <h1 class="h3 mb-0 text-gray-800">
            <i class="fas fa-receipt text-danger"></i> <?= esc($title ?? 'Finanzas — Egresos') ?>
        </h1>
        <div>
            <?php if ($can_draft ?? false): ?>
            <button class="btn btn-danger btn-sm mr-2" onclick="showModal()">
            ...
            <div class="btn-group d-inline-block">
                ...
                <div class="dropdown-menu">
                    <a class="dropdown-item" href="<?= base_url('/app/finance/expenses_detail') ?>">Todos</a>
                    <?php foreach ($expense_types as $key => $label): ?>
                    <a class="dropdown-item <?= ($current_type ?? '') === $key ? 'active' : '' ?>"
                       href="<?= base_url('/app/finance/expenses_detail?type=' . $key) ?>">
                        <?= esc($label) ?>
                    </a>
                    <?php endforeach; ?>
                </div>
    ...
    <div class="card shadow mb-4 border-left-warning" id="pendingCard" style="display:none;">
        <div class="card-header py-3">
            <h6 class="m-0 font-weight-bold text-warning"><i class="fas fa-clock"></i> Egresos Pendientes de Aprobación</h6>
        </div>
        <div class="card-body">
    ...
    <div class="card shadow mb-4">
        <div class="card-header py-3">
            <h6 class="m-0 font-weight-bold text-primary">
                ...
                <?= $current_type ? esc($expense_types[$current_type] ?? 'Desconocido') : 'Todos los Gastos' ?>
            </h6>
        </div>
        <div class="card-body">
            <div class="table-responsive">
                <table id="expenseDetailTable" class="table table-striped table-bordered" style="width:100%">
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header bg-gradient-danger text-white">
                <h5 class="modal-title"><i class="fas fa-plus-circle"></i> Nuevo Egreso</h5>
            ...
            <form id="expenseForm">
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" name="description" placeholder="Detalle del egreso">
                    </div>
                    <div class="form-group">
                        <label>Notas</label>
                ...
                <div class="modal-footer">
                    ...
                    <button type="submit" class="btn btn-danger"><i class="fas fa-save"></i> Registrar</button>
                </div>
            </form>
        </div>
    </div>
...
<script>
var dt, catalog = null;
var apiBase = '<?= base_url('/app/finance/expenses_detail/api') ?>';
...
function loadTable() {
    $.post(apiBase + '/list', { type: currentType, date_from: dateFrom, date_to: dateTo }, function(response) {
        ...
        if (dt) dt.clear().rows.add(rows).draw();
            ...
            dt = $('#expenseDetailTable').DataTable({
                data: rows,
                pageLength: 25,
...
function loadPending() {
    ...
    $.post(pendingUrl, function(r) {
        var rows = (r.data || []).filter(function(m) { return m.workflow_type === 'egreso'; });
...
function reject(id) {
    if (!confirm('¿Rechazar este egreso?')) return;
