        ...
        <div class="card h-100">
            <div class="card-body">
                ...
                <div class="form-group">
                    ...
                    <?= form_dropdown('area_type', array_column($area_type, 'name', 'id'), $property_data['area_type'] ?? '', 'class="select2-single form-control" id="select2Single_area_type" disabled required') ?>
                </div>
                <div class="form-group">
                    <?= form_label('Tipo de vivienda', 'housing_type'); ?>
                    <?= form_dropdown('housing_type', array_column($housing_type, 'name', 'id'), $property_data['housing_type'] ?? '', 'class="select2-single form-control" id="select2Single_housing_type" disabled required') ?>
                </div>
                <div class="form-group">
                    <?= form_label('Modelo de negocio', 'business_model'); ?>
                    <?= form_dropdown('business_model', array_column($business_model, 'name', 'id'), $property_data['business_model'] ?? '', 'class="select2-single form-control" id="select2Single_business_model" disabled required') ?>
                </div>
                <div class="form-group">
                    <?= form_label('Tipo de mercado', 'market_type'); ?>
                    <?= form_dropdown('market_type', array_column($market_type, 'name', 'id'), $property_data['market_type'] ?? '', 'class="select2-single form-control" id="select2Single_market_type" disabled required') ?>
                </div>
                <div class="form-group">
                    <?= form_label('Habitaciones', 'bedrooms'); ?>
                    <?= form_input('bedrooms', $property_data['bedrooms'], 'class="form-control rounded" disabled') ?>
                </div>
                <div class="form-group">
                    <?= form_label('Baños', 'bathrooms'); ?>
                    <?= form_input('bathrooms', $property_data['bathrooms'], 'class="form-control rounded" disabled') ?>
                </div>
                <div class="form-group">
                    <?= form_label('Puestos de estacionamiento', 'garages'); ?>
                    <?= form_input('garages', $property_data['garages'], 'class="form-control rounded" disabled') ?>
                </div>
                <div class="form-group">
                    <?= form_label('Construcción m²', 'meters_construction'); ?>
                    <?= form_input('meters_construction', $property_data['meters_construction'], 'class="form-control rounded" disabled') ?>
                </div>
                <div class="form-group">
                    <?= form_label('Terreno m²', 'meters_land'); ?>
                    <?= form_input('meters_land', $property_data['meters_land'], 'class="form-control rounded" disabled') ?>
                </div>
                <div class="form-group">
                    <?= form_label('Estado', 'state'); ?>
                    <?= form_dropdown('state', array_column($state, 'name', 'id'), $property_data['state'] ?? '', 'class="select2-single form-control" id="select2Single_state" disabled required') ?>
                </div>
                <div class="form-group">
                    <?= form_label('Municipio', 'municipality'); ?>
                    <?= form_dropdown('municipality', array_column($municipality, 'name', 'id'), $property_data['municipality'] ?? '', 'class="select2-single form-control" id="select2Single_municipality" disabled required') ?>
                </div>
                <div class="form-group">
                    <?= form_label('Ciudad/Zona:', 'city'); ?>
                    <?= form_dropdown('city', array_column($city, 'name', 'id'), $property_data['city'] ?? '', 'class="select2-single form-control" id="select2Single_city" disabled required') ?>
                </div>
                <div class="form-group">
                    <?= form_label('Dirección (30 Caracteres)', 'address'); ?>
                    <?= form_input('address', $property_data['address'] ?? '', 'class="form-control rounded" disabled') ?>
                </div>
                <div class="form-group">
                    <?= form_label('Precio para venta', 'price'); ?>
                    <?= form_input('price', $property_data['price'] ?? '', 'class="form-control rounded" disabled') ?>
                </div>
                <div class="form-group">
                    <?= form_label('Precio de alquiler', 'price_additional'); ?>
                    <?= form_input('price_additional', $property_data['price_additional'] ?? '', 'class="form-control rounded" disabled') ?>
                </div>
                <div class="form-group">
                    <?= form_label('Propietario', 'owner'); ?>
                    <?= form_input('owner', $property_data['owner'], 'class="form-control rounded" disabled') ?>
                </div>
                <div class="form-group">
                    <?= form_label('Correo del propietario', 'owner_mail'); ?>
                    <?= form_input('owner_mail', $property_data['owner_mail'], 'class="form-control rounded" disabled') ?>
                </div>
                <div class="form-group">
                    <?= form_label('Teléfono del propietario', 'owner_phone'); ?>
                    <?= form_input('owner_phone', $property_data['owner_phone'], 'class="form-control rounded" disabled') ?>
                </div>
                <hr class="mt-5">
                <div class="form-group pt-5">
                    ...
                    <p>
                        De necesitar modificar los datos previamente cargados, deberás dirigirte a la gerencia de ventas y solicitarlo.
                    </p>
                </div>
            </div>
        </div>
    ...
    <div class="col-sm-12">
        <div class="card">
            <div class="card-body">
                <div class="d-flex justify-content-between">
                    <h4 class="pt-4">
                        Análisis del bien raíz
                    </h4>
                    <h4 class="pt-4">
                        Porcentaje:
                        <strong>
                            <?= $checklistManage[0] > 0 ? '%'.round((100 / count($checklist) * count($checklistManage)), 2) : 'Sin porcentaje' ?>
                        </strong>
                    </h4>
                </div>
                <hr>
                <form action="<?= base_url('/app/my_properties/property_update/analysis/'.$id_property) ?>" method="post">
                    <?= csrf_field() ?>
                    <div class="row">
                    <?php 
                        $totalElements = count($checklist);
                        $valuePerElement = 100 / $totalElements;
                            ...
                            <div class="form-group">
                                <?php 
                                    $data = array(
                                        ...
                                        'id'          => 'check_'.$check['id'],
                                        'value'       => $check['id'],
                                        'checked'     => in_array($check['id'], $checklistManage) ? true : false,
                                    );
                                ?>
                                <?= form_checkbox($data) ?>
                                <?= form_label($check['name'].' <strong>(Ponderación %'.round($valuePerElement, 2).')</strong>', 'check_'.$check['id']); ?>
                            </div>
                        </div>
                    <?php endforeach; ?>
                    </div>
