    ...
    return (
        '<table cellpadding="5" cellspacing="0" border="0" style="padding-left:50px;">' +
            ...
            '<tr>' +
                ...
                '<td>' +
                    '<a class="btn btn-primary btn-sm" href="/assigned_clients/edit_assigned_clients/'+
                    d.id_lead +
...
$(document).ready(function () {
    var table = $('#assigned_clients_table').DataTable({
        pageLength : 100,
        scrollX: true,
        scroller: {
            rowHeight: 60
        ...
        createdRow: function(row, data, dataIndex) {
            ...
            if (day_difference(data['created_at_lead']) >= 2) {
                if (data['name_tracking_assigned_client']) {
                    $(row).find('td:eq(9)').css('background-color', '#28a745').css('color', '#fff');
                    $(row).find('td:eq(10)').css('background-color', '#28a745').css('color', '#fff');
                }else{
                    $(row).find('td:eq(9)').css('background-color', '#dc3545').css('color', '#fff');
                    $(row).find('td:eq(10)').css('background-color', '#dc3545').css('color', '#fff');
                }
            }
            else {
                $(row).find('td:eq(9)').css('background-color', '#28a745').css('color', '#fff');
            ...
            data['name_tracking_assigned_client'] ? '' : $(row).find('td:eq(6)').css('color', '#ff0854');
            data['name_assignedclients_assigned_client'] ? '' : $(row).find('td:eq(6)').css('color', '#ff0854');
            data['first_contact_at_assigned_client'] ? '' : $(row).find('td:eq(8)').css('color', '#ff0854');
        }
        
    });

    // Add event listener for opening and closing details
    $('#assigned_clients_table tbody').on('click', 'td.dt-control', function () {
